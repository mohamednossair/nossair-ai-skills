---
name: angular
description: Review or develop Angular 20+ standalone components, signals, and modern template control flow
---

# Angular 20+ Skill

Use this skill when developing or reviewing Angular components, services, and templates. This project follows Angular 20+ standards with standalone components and signal-based reactivity.
...
- Use `inject()` for all dependency injection: `private userService = inject(UserService);`.
...
- `resource()` API: declarative async data loading (replaces many manual subscribe patterns).
- `toSignal(observable$)`: bridge RxJS Observables to Signals.
...
- Use `@defer` for lazy-loading heavy components: `@defer (on viewport) { <app-heavy /> }`.
...
@Component({
  ...
  template: `
    @for (user of users.value(); track user.id) {
      <div (click)="selected.emit(user)">{{ user.name }}</div>
    } @empty {
      <p>No users found.</p>
    }
  `
